...
Beyond setting the desired mesh size via the ``Nmesh`` parameter,
the :func:`~CatalogSource.to_mesh` function takes several additional parameters that
the user can use to configure the mesh.
...
.. note::
    ...
    See :attr:`pmesh.window.methods` for the full list of supported
    window kernels.
...
Compensation: Correcting the Window Kernel
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
using the :class:`~nbodykit.source.mesh.memory.MemoryMesh` class. For example,
...
.. ipython:: python
    ...
    real_mesh = MemoryMesh(rfield)
    print(mesh)

    cfield = rfield.r2c()
    complex_mesh = MemoryMesh(cfield)
