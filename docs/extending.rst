...
section, we'll describes the details of implementing plugins for the 4 built-in
...
the plugin. The main function of :func:`register` is to update this schema object 
...
A few things to note in this example:
    ...
    3. Any default values for parameters and whether or not the parameter is required will be directly inferred from the :func:`__init__` calling signature.
    4. Parameters in a plugin's schema will be automatically added to the class instance before the body of :func:`__init__` is executed -- the user does not need to re-set these attributes. As such, the body of :func:`__init__` in this example is empty. However, additional initialization-related computations could also be performed here.
