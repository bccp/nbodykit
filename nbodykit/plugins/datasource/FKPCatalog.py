...
def nbar_fromfile(filename):
    ...
    return spline(d[:,0], d[:,1])
 
class FKPCatalogDataSource(DataSource):
    """
    ...
    def __init__(self, data, randoms, 
        ...
        if self.cosmo is None:
            raise ValueError("please specify a input Cosmology to use in '%s'" %self.plugin_name)
    
        # initialize before bcasting 
        self.mean_coordinate_offset = None
        self.alpha = None
                
        # open the data sources
        defaults = {'Redshift':-1., 'Nbar':-1., 'Weight':1.}
        self._data_stream = self.data.open(defaults=defaults)
        self._randoms_stream = self.randoms.open(defaults=defaults)
        ...
        redshifts = []
        for [pos, z, nbar] in self._randoms_stream.read(['Position', 'Redshift', 'Nbar'], full=False):
            ...
            if len(pos):
                ...
                # store redshifts
                if self.nbar is None and nbar[0] != -1:
                    if z[0] != -1:
                        raise ValueError("if `nbar` not provided, randoms `DataSource` must supply `Redshift` column")
                    redshifts += list(z)
